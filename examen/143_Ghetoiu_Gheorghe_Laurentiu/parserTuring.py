...
def parser():
    ...
    q_acc = []
    q_rej = []
    q_ver = []  # q pentru verificarea identicitatii celor doua benzi

    ls_d = []  # lista cu elementele liste de 7 elemente pentru functia de tranzitie
    fin = open("ex2_config_file")
    if sys.argv[0] == "ex3.py":
        fin = open("ex3_config_file")
    if(len(sys.argv) >= 2):
        fin = open(sys.argv[1])
    ...
    return Q,sigma,gamma, tranzitii, q0, q_acc, q_rej, q_ver